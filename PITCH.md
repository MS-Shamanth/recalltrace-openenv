...
### [1:10–1:45] The Live Demo & LLM Upgrade

Let me show you what the learning actually looks like. If you go to our Hugging Face Space right now, you won't just see plots — you'll see a live interactive dashboard.

We didn't stop at RL. We took the expert demonstrations generated by our self-play engine and used them to fine-tune a 4-bit Large Language Model (`Qwen2.5-0.5B-Instruct`) via Unsloth and TRL. 

On our dashboard, under the **🤖 Live LLM Demo** tab, you can click "Run Simulation" and watch this fine-tuned model investigate a supply-chain graph in real-time on a T4 GPU. You can see its raw JSON thoughts as it traces lots, inspects crossdocks, and decides when its probabilistic belief is high enough to quarantine. 

### [1:45–2:15] Results

*[Show selfplay_training.png]*

Looking at the underlying engine: F1 score goes from 0.24 to 0.79 over 200 episodes. Nodes quarantined drops from 8.3 per episode to 3.1. Steps to finalize drops from 25 to 11. The adversary's reward flips from positive — it was winning — to negative — the investigator caught up.
...
The entire self-play loop runs in under one second on CPU, generating the perfect expert dataset for our LLM.
